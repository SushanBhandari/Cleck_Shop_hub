...
begin 
    ...
    select count(*) into v_count_wed from collection_slot  where Slot_Date in(v_next_wednesday);
    select count(*) into v_count_thu from collection_slot  where Slot_Date in(v_next_thursday); 
    select count(*) into v_count_fri from collection_slot  where Slot_Date in(v_next_friday); 

 
 
    if v_count_wed = 0 then 
